...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const fbApp = getApps().length ? getApps()[0] : initializeApp({
      projectId: process.env.FIREBASE_PROJECT_ID || 'hsus-gourmet',
  ...
  const adminEmails = (process.env.NOTIFY_EMAILS || 'yshahn@gmail.com,ymhahn@gmail.com').split(',').map(e => e.trim());
