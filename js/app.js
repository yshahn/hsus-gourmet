...
async function confirmReservation() {
  // Check if reservations are paused (this check was missing entirely —
  // the admin dashboard's "Pause Reservations" toggle wrote to Firestore
  // but nothing on this page ever read it before saving a reservation).
  try {
    const fbUrl = window.location.origin + '/js/firebase-menu.js';
    const { db } = await import(fbUrl);
    const { doc, getDoc } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js');
