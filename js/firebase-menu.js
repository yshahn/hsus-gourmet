...
const authInst    = getAuth(firebaseApp);
const MENU_DOC    = doc(db, 'config', 'menu');
export { db, firebaseApp };

// ── Admin panel uses a simple email-allowlist login (not real Firebase Auth),
// so Firestore writes need SOME authenticated session to satisfy security rules.
// This signs the admin panel in anonymously — no password needed — just to get
// a valid request.auth for writes. Safe to call repeatedly; resolves instantly
// if already signed in.
export function ensureAdminAuth() {
  return new Promise((resolve) => {
    if (authInst.currentUser) { resolve(authInst.currentUser); return; }
    const unsub = onAuthStateChanged(authInst, (user) => {
      if (user) { unsub(); resolve(user); }
    });
    signInAnonymously(authInst).catch(e => { console.error('Anonymous auth error:', e); resolve(null); });
  });
}

// ── Save the list of admin-access emails to Firestore
export async function saveAdminEmailsToFirebase(emails) {
  try {
    const adminDoc = doc(db, 'config', 'adminEmails');
    await setDoc(adminDoc, { data: JSON.stringify(emails), updatedAt: Date.now() });
    return true;
  } catch(e) { console.error('Admin emails save error:', e); return false; }
}

// ── Load the list of admin-access emails from Firestore
export async function loadAdminEmailsFromFirebase() {
  try {
    const adminDoc = doc(db, 'config', 'adminEmails');
    const snap = await getDoc(adminDoc);
    if (snap.exists()) return JSON.parse(snap.data().data);
  } catch(e) { console.error('Admin emails load error:', e); }
  return null;
}
...
// ── Listen for real-time reservation updates (new bookings, guest Yes/No responses, etc.)
...
// ── Load service ON/OFF status from Firestore (orders/reservations pause
// toggle written by the admin dashboard). Fields are stored directly on
// the doc (not JSON-string-wrapped), matching how admin.html writes them.
export async function loadServicesFromFirebase() {
  try {
    const svcDoc = doc(db, 'config', 'services');
    const snap = await getDoc(svcDoc);
    if (snap.exists()) return snap.data();
  } catch(e) { console.error('Services load error:', e); }
  return null;
}

// ── Live-listen for service ON/OFF changes (guest-facing checks update
// instantly and stay correct without needing a fresh fetch on every click)
export function listenServices(callback) {
  const svcDoc = doc(db, 'config', 'services');
  return onSnapshot(svcDoc, snap => {
    if (snap.exists()) callback(snap.data());
  });
}

// ── Pickup settings (prep time, hours, scheduling) sync ────────────────
// Previously these lived ONLY in the admin's own browser localStorage —
// changing "Prep time" in admin settings updated nothing anyone else could
// see. Guest checkout, email/SMS text, and the admin Orders list all show
// whatever string the guest's own browser generated at order time, using
// its OWN stale localStorage default. Syncing through Firestore (like the
// orders/reservations pause toggle) fixes this for everyone at once.
export async function loadPickupSettingsFromFirebase() {
  try {
    const psDoc = doc(db, 'config', 'pickupSettings');
    const snap = await getDoc(psDoc);
    if (snap.exists()) return snap.data();
  } catch(e) { console.error('Pickup settings load error:', e); }
  return null;
}

export async function savePickupSettingsToFirebase(settings) {
  try {
    await setDoc(doc(db, 'config', 'pickupSettings'), settings, { merge: true });
    return true;
  } catch(e) { console.error('Pickup settings save error:', e); return false; }
}

export function listenPickupSettings(callback) {
  const psDoc = doc(db, 'config', 'pickupSettings');
  return onSnapshot(psDoc, snap => {
    if (snap.exists()) callback(snap.data());
  });
}
