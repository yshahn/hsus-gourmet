...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  ...
  if (isReservation) {
    ...
    // SMS is kept short and emoji/special-character-free on purpose: emoji
    // and characters outside GSM-7 (—, ·, etc.) force UCS-2 encoding (70
    // chars/segment instead of 160), and long itemized orders were
    // exceeding the 10-segment carrier limit — AT&T rejects those outright
    // while other carriers happened to let them through. Full itemized
    // details still go out via email and are visible in the admin
    // dashboard, so nothing is lost — the SMS is just a short heads-up.
    const itemCount = orderItems.length;
    adminSmsText = [
      `New order - Hsu's Gourmet`,
      `${customer?.name || 'Guest'} (${phone}) - ${itemCount} item${itemCount === 1 ? '' : 's'} - $${(total || 0).toFixed(2)}`,
      `Pickup: ${pickup}${curbsideTxt}`,
      specialReq ? `Note: ${specialReq}` : '',
      `Full order: check email or admin dashboard.`,
    ].filter(Boolean).join('\n');
    ...
    guestSmsText = [
      ...
      `${itemCount} item${itemCount === 1 ? '' : 's'} - Total: $${(total || 0).toFixed(2)}`,
      specialReq ? `Note: ${specialReq}` : '',
      `Full receipt sent to your email. Questions? (404) 577-0888${arrivedLink}`,
    ].filter(Boolean).join('\n');
