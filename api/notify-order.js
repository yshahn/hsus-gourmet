...
// Sends email via Resend + SMS via Telnyx
// To: Admin (from settings) + Guest (confirmation)

const { sendSMS, sendAdminSMS } = require('../lib/sms');

// Formats a phone number for DISPLAY in emails/SMS text — e.g. "+14045551234"
// or "4045551234" becomes "(404) 555-1234". This is display-only: the raw
// E.164 value (with +1) is still what's used to actually dial/send SMS —
// only the text shown to a human reader gets reformatted.
...
module.exports = async function handler(req, res) {
  ...
  if (req.body.isArrived) {
    ...
    const custPhone = formatPhoneDisplay(customer?.phone);
    ...
    // ── SMS notification (Telnyx) ──
    const smsText = `Arrived for curbside pickup! Vehicle: ${carInfo}. Phone: ${custPhone}${orderId ? '. Order #' + orderId : ''}`;

    if (adminPhones.length > 0) {
      try {
        const results = await sendAdminSMS(adminPhones, smsText);
        arrivedResults.sms = { ok: results.every(r => r.ok) };
        console.log('Arrived SMS (Telnyx):', JSON.stringify(results.map(r => r.ok)));
  ...
  // ── ORDER SAVE FAILURE ALERT ──
  // Sent when the browser's saveOrderToFirebase kept failing even after
  // retries. The order was paid and the customer already got their normal
  // confirmation email/SMS — this is a SEPARATE, admin-only heads-up that
  // it never made it into Firestore/the admin dashboard, so someone should
  // check the payment and enter it manually if needed.
  if (req.body.isSaveFailureAlert) {
    const { orderId, customer, total, pickupTime, notifEmails, notifPhones } = req.body;
    const adminEmails = (notifEmails && notifEmails.length ? notifEmails : ['yshahn@gmail.com', 'ymhahn@gmail.com', 'hsusp192@gmail.com']);
    const adminPhones = (notifPhones && notifPhones.length ? notifPhones : ['+17705008420', '+16788629389']);
    const custName = customer?.name || 'Guest';
    const custPhone = formatPhoneDisplay(customer?.phone);
    ...
    try {
      await sendResendEmail({
        to: adminEmails,
        subject: `⚠️ Order NOT saved to dashboard - Hsu's Gourmet`,
        html: `<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;padding:24px;">
          <h2 style="color:#c0392b;">⚠️ Order save failed</h2>
          <p>A customer's order was paid and they received a normal confirmation, but it failed to save to the admin dashboard after multiple retries. Please check manually.</p>
          <p><b>Order #:</b> ${orderId || 'unknown'}<br/>
          <b>Customer:</b> ${custName} (${custPhone})<br/>
          <b>Total:</b> $${parseFloat(total || 0).toFixed(2)}<br/>
          <b>Pickup:</b> ${pickupTime || ''}</p>
        </div>`,
      });
    } catch(e) { console.error('Save-failure admin email error:', e); }

    try {
      const smsText = `ALERT: Order NOT saved to dashboard. ${custName} (${custPhone}) - $${parseFloat(total || 0).toFixed(2)} - check Stripe/manually enter.`;
      await sendAdminSMS(adminPhones, smsText);
    } catch(e) { console.error('Save-failure admin SMS error:', e); }

    return res.status(200).json({ success: true, type: 'save_failure_alert' });
  }
...
// ── CONFIRMATION REQUEST ──
  if (req.body.isConfirmationRequest) {
    ...
    const baseUrl = 'https://hsusgourmet.com';
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f9f9f9;">
  <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
    ...
    if (guestPhone) {
      // SMS uses "Reply 1 to CONFIRM or 2 to CANCEL" — matching the sample
      // message registered with the shared Telnyx 10DLC campaign (all
      // three restaurants share one campaign/number). The link-based
      // Yes/No flow (shortYes/shortNo) is kept for email only.
  ...
  // ── Send SMS via Telnyx ──
  console.log('Telnyx check:', !!process.env.TELNYX_API_KEY, !!process.env.TELNYX_FROM_PHONE);
  console.log('Resend check:', !!RESEND_KEY);

  // Admin SMS
  try {
    const adminResults = await sendAdminSMS(adminPhones, adminSmsText);
    adminResults.forEach((r, i) => {
      console.log('Admin SMS:', adminPhones[i], r.ok ? 'sent' : 'failed');
      results.smsAdmin.push({ to: adminPhones[i], ok: r.ok });
    });
  ...
  if (guestPhone && guestPhone !== '—' && req.body.guestSmsConsent) {
    try {
      const r = await sendSMS(guestPhone, guestSmsText);
      console.log('Guest SMS:', r.ok ? 'sent' : 'failed');
