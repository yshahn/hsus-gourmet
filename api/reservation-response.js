...
async function sendEmail({ to, subject, html }) {
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: "Hsu's Gourmet <orders@pacificrimatl.com>", to: toList, subject, html }),
    });
    return r.ok;
...
module.exports = async function handler(req, res) {
  ...
  // like a real click, which is why every real click looked like 2 hits.
  ...
  // ── Reservation date already passed? ──
  // A guest clicking an old confirmation-request email days after the
  // reservation date has passed should produce NO notification to
  // anyone — it's moot, the same as a late SMS reply on the shared-number
  // flow. Without this, a stale link click still fired a normal-looking
  // "CONFIRMED" alert for a reservation that already came and went.
  const resDateParsed = date ? new Date(date) : null;
  if (resDateParsed && !isNaN(resDateParsed.getTime())) {
    const todayStart = new Date(); todayStart.setHours(0, 0, 0, 0);
    if (resDateParsed.getTime() < todayStart.getTime()) {
      console.warn('[reservation-response] Link click for a past reservation date, ignoring silently:', date);
      res.setHeader('Content-Type', 'text/html');
      return res.status(200).send('<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;text-align:center;padding:60px 24px;"><p>This reservation has already passed. If you\'d like to book a new one, please call us at (404) 659-2788 or visit our website.</p></body></html>');
    }
  }

  // ── Idempotency guard (atomic) ──
  // Link-scanning bots (email security filters, SMS link previews, etc.) often
  // "click" action links automatically before the guest ever taps them — and
  // sometimes several of these hit the server within milliseconds of each
  // other. A plain "read then write" check has a race condition: two nearly-
  // simultaneous requests can both read the OLD status before either one
  // writes, so both think they're "first" and both send notifications. A
  // Firestore transaction closes that gap — only one request can win.
  ...
  // avoids the 4-5x duplicate notification issue from link-scanning bots.
  if (!alreadyProcessed) {
  const adminEmails = (process.env.NOTIFY_EMAILS || 'yshahn@gmail.com,ymhahn@gmail.com,hsusp192@gmail.com').split(',').map(e => e.trim());
...
<title>${isConfirmed ? 'See you soon!' : 'Reservation Cancelled'} — Hsu's Gourmet</title>
<style>
...
<body>
  <div class="card">
    ...
    <p>Questions? Call us at <span class="phone">(404) 659-2788</span></p>
  </div>
</body></html>`;
